          ...
          @if ($payment_details)
                ...
                @if (empty($payments) || $payments->payment_status != 'success' && $payments->payment_status != 'free')
                    ...
                    <button type="button"  class="btn btn-primary mr-5 m-l-15 razorpay-payment-button" id="pay_later">
                        <a href="{{route('pay-later',[$student_id, $program_data->id,Crypt::encrypt($program_data->application_fee)])}}" class="text-light">
                            Pay Later
                        </a>
                    </button>
                    <button type="button" id="paybtn" class="btn btn-primary mr-4 m-l-15 razorpay-payment-button loading_button">
                        <span style="display: none;" class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
                        <span><a href="{{ route('pay-amount', [$student_id, $program_data->id, Crypt::encrypt($program_data->application_fee)]) }}" class="text-light">Pay and Continue</a></span>
                    </button>
                    @else
                        <button type="button" id="paybtn" class="btn btn-primary mr-4 m-l-15 razorpay-payment-button loading_button">
                            <span style="display: none;" class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
                            <span><a href="{{ route('continue-course', [$student_id, $program_data->id, Crypt::encrypt($program_data->application_fee)]) }}" class="text-light">Continue</a></span>
                        </button>
                    @endif
                @endif
            @else
