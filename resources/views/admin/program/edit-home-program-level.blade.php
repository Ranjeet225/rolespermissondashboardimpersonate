                ...
                <form class="row g-4"  action="{{route('update-home-program-level',$home_program->id)}}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="col-12">
                        <div class="form-floating">
                           <select class="form-control sidfrm" name="program_level_id" id="lead-programCampus" placeholder="Courses Campus">
                              <option value="">-- Select programLevel--</option>
                              @foreach ($program_level as $item)
                                 <option value="{{$item->id}}" {{ $home_program->program_level_id == $item->id ? 'selected' : '' }}>{{$item->name}}</option>
                              @endforeach
                           </select>
                    ...
                    <div class="col-12">
                       <label>Description</label>
                        <textarea name="description" id="summernote1" cols="30" rows="10">{{$home_program->description}}</textarea>
                        @error('description')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
